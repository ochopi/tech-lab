...
set "LOG_MAXSIZE=102400"        :: 100KB en bytes
set "LOG_RETENTION_DAYS=60"
set "DEF_U=grupovasport\TPV02"
set "DEF_P=BuenaTPV02"
...
REM Borramos archivos de log viejos
goto :LOG_Cleanup

REM Devolvemos la ejecución al inicio del script
:LOG_Startup
...
if exist \\!IPServer1!\SVI$\ (
    ...
    net use \\!IPServer1!\SVI$\ !DEF_P! /USER:!DEF_U! /persistent:yes
    if exist \\!IPServer1!\SVI$\ (
        set "ServerIP=%IPServer1%"
        echo Fuente: \\!ServerIP!\SVI$\ >> "%LOGFILE%"
    ) else (
        net use \\!IPServer2!\SVI$\ !DEF_P! /USER:!DEF_U! /persistent:yes
        if exist \\!IPServer2!\SVI$\ (
            set "ServerIP=%IPServer2%"
            echo Fuente: \\!ServerIP!\SVI$\ >> "%LOGFILE%"
        ) else (
...
if exist "%SCRIPT_ROOT%\comandos.cmd" (
    ...
    start /MIN "%SCRIPT_ROOT%\comandos.cmd"
    echo comandos.cmd terminado (codigo: %ERRORLEVEL%) >> "%LOGFILE%"
) else (
...
:: Si supera tamaño, rotar log
if !SIZE! GEQ %LOG_MAXSIZE% (
    echo Rotando log: !SIZE! bytes
    for /f %%i in ('powershell -NoProfile -Command "Get-Date -Format yyyyMMdd_HHmmss"') do set "DATESTAMP=%%i"
    set "OLDLOG=%SCRIPT_ROOT%\log_!DATESTAMP!.txt"
    move "%LOGFILE%" "!OLDLOG!"
    echo. > "%LOGFILE%"
)

:: Borrar logs viejos (mayores a 60 días)
forfiles /p "%SCRIPT_ROOT%" /m "log_*.txt" /d -%LOG_RETENTION_DAYS% /c "cmd /c del @path"

:: EJEMPLO: escribir algo en el log
echo [%DATE% %TIME%] Script Inicializado >> "%LOGFILE%"

:: Devolvemos la ejecución al inicio del script
goto :LOG_Startup
