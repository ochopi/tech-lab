...
REM ============ PROBAR CONEXIÓN SECUENCIAL ==========
set "ServerIP="
...
if not errorlevel 1 (
    echo Ping exitoso a !Server61! >> "%LOGFILE%"
    dir "\\!Server61!\SVI$" >nul 2>&1
    if not errorlevel 1 (
        echo Acceso exitoso a \\!Server61!\SVI$ >> "%LOGFILE%"
        set "ServerIP=!Server61!"
    ) else (
        echo No se puede acceder a \\!Server61!\SVI$ >> "%LOGFILE%"
    )
) else (
...
if not defined ServerIP (
    ...
    if not errorlevel 1 (
        echo Ping exitoso a !Server31! >> "%LOGFILE%"
        dir "\\!Server31!\SVI$" >nul 2>&1
        if not errorlevel 1 (
            echo Acceso exitoso a \\!Server31!\SVI$ >> "%LOGFILE%"
            set "ServerIP=!Server31!"
        ) else (
            echo No se puede acceder a \\!Server31!\SVI$ >> "%LOGFILE%"
        )
    ) else (
...
if not defined ServerIP (
    ...
    if not errorlevel 1 (
        echo Ping exitoso a %DEFAULT_SERVER1% >> "%LOGFILE%"
        dir "\\%DEFAULT_SERVER1%\SVI$" >nul 2>&1
        if not errorlevel 1 (
            echo Acceso exitoso a \\%DEFAULT_SERVER1%\SVI$ >> "%LOGFILE%"
            set "ServerIP=%DEFAULT_SERVER1%"
        ) else (
            echo No se puede acceder a \\%DEFAULT_SERVER1%\SVI$ >> "%LOGFILE%"
        )
    ) else (
...
if not defined ServerIP (
    ...
    if not errorlevel 1 (
        echo Ping exitoso a %DEFAULT_SERVER2% >> "%LOGFILE%"
        dir "\\%DEFAULT_SERVER2%\SVI$" >nul 2>&1
        if not errorlevel 1 (
            echo Acceso exitoso a \\%DEFAULT_SERVER2%\SVI$ >> "%LOGFILE%"
            set "ServerIP=%DEFAULT_SERVER2%"
        ) else (
            echo No se puede acceder a \\%DEFAULT_SERVER2%\SVI$ >> "%LOGFILE%"
        )
    ) else (
